...
import { useWeeklyWorldPrefetch } from "@/features/reflection/hooks";
 ...
 * 内部では useWeeklyWorldPrefetch を使用。
 *
 * @example
 * ```tsx
 ...
 * if (hasWorld) {
 *   return <Image source={{ uri: weeklyWorldImageUrl }} />;
 * }
 * return <Placeholder />;
 * ```
 */
export const useCurrentWeekWorld = (): CurrentWeekWorldReturn => {
  ...
  const { weeklyWorld, isLoading, error, notFound } =
    useWeeklyWorldPrefetch(currentWeekStart);
