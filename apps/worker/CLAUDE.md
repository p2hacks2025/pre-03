...
| ジョブ | 実行タイミング | 内容 |
|--------|---------------|------|
| `ai-post-short-term` | 5分ごと | 直近投稿への反応 + スタンドアロン（2%） |
| `ai-post-long-term` | 毎日3:00 | 過去投稿（7日以上前）への反応（50%でジョブ実行） |

### スタンドアロン投稿

ユーザー投稿に関係なく、AIが自発的に生成する投稿。

- **short-term のみ**: 5分ごとに2%確率で生成
- long-term ではスタンドアロン投稿なし

### 頻度制限

- **ユーザーごと**: 1時間に最大5件までAI投稿を受ける
- 全体の上限はなし（各ユーザーの制限のみ）
- short-term / long-term 両方に適用
...
1. **スタンドアロン投稿**: 2%確率でランダム投稿を生成
2. **24時間分のユーザーポストを1回で取得**（DBアクセス最適化）
3. **ユーザーごとにループ**
   - **ユーザー頻度チェック**: 5件/時間を超えていればスキップ
...
1. **ジョブ実行判定**: 50%確率でジョブ全体をスキップ
2. **過去投稿があるユーザーID一覧を取得**（7日以上前）
3. **ユーザーごとにループ**
   - **ユーザー頻度チェック**: 5件/時間を超えていればスキップ
   - **50%確率判定**: 失敗ならスキップ
   - **ランダム2件の過去投稿を取得**
...
- `AI_POST_CONFIG.LONG_TERM_USER_CHANCE`: ユーザーごとの確率（デフォルト: 0.5）
- `AI_POST_CONFIG.LONG_TERM_POSTS_PER_USER`: ユーザーごとの投稿数（デフォルト: 2）
- `AI_POST_CONFIG.MAX_POSTS_PER_HOUR`: ユーザーごとの頻度制限（デフォルト: 5）
