...
│   │   ├── ai-posts.ts        # AI投稿テーブル定義
│   │   ├── ai-profiles.ts     # AIプロフィールテーブル定義
│   │   ├── user-posts.ts      # ユーザー投稿テーブル定義
│   │   ├── user-profiles.ts   # ユーザープロフィールテーブル定義
│   │   ├── weekly-worlds.ts   # 週間ワールドテーブル定義
│   │   ├── world-build-logs.ts # ワールド構築ログテーブル定義
│   │   └── relations.ts       # テーブル間リレーション
│   └── seed/
│       ├── index.ts           # シードエントリーポイント
│       ├── runner.ts          # シード実行エンジン
│       ├── assets/            # シード用アセット（画像・プロンプト）
│       ├── seeders/
│       │   ├── index.ts       # Seeder型定義
│       │   ├── storage.ts     # [Infrastructure] Storageバケット作成
│       │   ├── users.ts       # [Data] テストユーザー作成
│       │   ├── posts.ts       # [Data] 投稿データ作成
│       │   ├── weekly-worlds.ts    # [Data] 週間ワールド作成
│       │   ├── test-weekly-worlds.ts # [Data] テスト用週間ワールド
│       │   └── worker-posts.ts     # [Data] ワーカー投稿データ
│       └── utils/
...
| 参考ファイル | 内容 |
|-------------|------|
| `src/schema/user-profiles.ts` | テーブル定義の例 |
| `src/schema/user-posts.ts` | 投稿テーブル定義の例 |
...
| パターン | 参考ファイル |
|---------|-------------|
| テーブル定義 | `src/schema/user-profiles.ts` |
| 外部キー参照 | `src/schema/user-profiles.ts`（authUsers 参照） |
| リレーション | `src/schema/relations.ts` |
| Infrastructure シーダー | `src/seed/seeders/storage.ts` |
| Data シーダー | `src/seed/seeders/users.ts` |
| 投稿シーダー | `src/seed/seeders/posts.ts` |
| 週間ワールドシーダー | `src/seed/seeders/weekly-worlds.ts` |
